...
**输出标签规范**（`[USER-FORMAT-01]`）：Seedance 的镜头标题用**裸标题** `## 镜头 N`（或英文 `## Shot N`），**不带说明后缀**——每镜的唯一情绪任务是内部设计纪律，写进该镜正文首句，不写进标题；对白行标签统一用 **`对白：`**（英文 Prompt 用 `Dialogue:`），不用“台词：”。Kling 的 `【0–t｜状态】` 分段框架不受此规范影响（状态名是该已验证框架的组成部分）。
