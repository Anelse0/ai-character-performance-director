...
**推荐输入:六槽创作 Brief**(源自 `[EXT-OBS-01]` 结构翻转;可由用户或上游 LLM 填写,全部槽位可选——填了的槽自动进入"输出前逐项核对"的硬要求清单,没填的按下方规则最小推断并说明假设):

```text
【素材参考】参考图/视频/音频各自绑定谁(身份/服装/场景/音色/动作)
【人物】外观一致性要求 + 人物小传(性格/历史/恐惧/信念;双人戏各写一份)
【场景】时代/地点/时刻/氛围/光 + 人物空间关系(距离及其变化)
【对白】台词原文(逐字锁定)+ 语言/口音/语速/delivery 要求
【镜头】构图与视角(越肩/正面/POV)/景别与焦段/运镜及其动机
【表演时间线】情绪弧(起点→触发→峰值→收束式)+ 词语级触发点(哪个词引爆什么)
```

自然语言输入同样接受。

内部归一化为语义对象，但不要强迫用户填写表格。以下是最常用的**核心字段**；交互、复杂运镜、Kling 动作控制、原地锁定等**扩展字段与完整清单见 `references/input-schema.md`**：
