...
### [USER-PROMPT-01] Prompt 自足性与必写项

- 状态：`user_approved`
- 用户要求日期：2026-08-17。
- 触发：实际调用中出现两处使用问题——(1)图生视频时误删场景/氛围/光/可见生理信号描述;(2)把"承接上一段/反打/同一个夜晚"等模型不可用的叙事上下文写进 Prompt 正文。
- 要求：(a)**每条 Prompt 独立自足**,模型对上一段视频与整体剧情无记忆;叙事承接/反打/上下文只用于内部设计,不进正文;连续性用显式可见状态(首帧、姿态、泪光、位置、未说完的话)与 extension/末帧表达。(b)**场景/氛围/光/可见生理信号是每条 Prompt 的必写项**,图生视频也不省略;i2v 只是不重复首帧已提供的空间几何,不删氛围、光与可见信号。
- 边界：这是用户批准的输出规则,不构成模型对场景或连续性稳定服从的保证。
