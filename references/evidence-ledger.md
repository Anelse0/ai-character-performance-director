...
- 禁止扩写：仅限该模型、时长、图参考与内容类型与**单一情绪递进**形态;**多情绪转折与双人交流形态目前无成片验证,为设计级(`experimental`)**;不外推为 Seedance 2.0 / Kling 3.0 或任意情绪场景的通用保证,换模型/时长/镜头/形态需重新验收。

### [USER-SD20-01] Seedance 2.0 连续 prose 承载情绪转折失败

- 状态：`user_verified`
- 用户报告日期：2026-08-17。
- 条件：Seedance 2.0、10 秒、单主体多情绪转折(维持体面→被戳破→破口→压回冷下来);Prompt 用**连续 causal prose**、非分镜结构。
- 用户结果：整体情绪转折"很假"。
- 允许结论:连续 prose 会把转折三段揉平,配合 10s 内回落无独立空间/无具体降温动作,导致转折不可读。**情绪表演应改用结构化分镜(= 用户验证格式)+ 回落单列一个 Shot 并写降温动作**,并符合官方 `[S20-FMT-01]`。
- 禁止扩写:该结论定位到 Prompt 格式与时长/回落设计,不据此断定 Seedance 2.0 不能做情绪转折,也不把某一措辞判为唯一原因;修正版需重新成片验收。
