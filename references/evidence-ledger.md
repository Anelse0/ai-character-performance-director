...
### [USER-FORMAT-01] Seedance 输出标签规范

- 状态：`user_approved`
- 用户要求日期：2026-08-18。
- 要求：(a)Seedance 镜头标题用**裸标题** `镜头 N` / `Shot N`,不带说明后缀;每镜情绪任务写进正文首句,不写进标题。(b)对白行标签统一 **`对白：`**(英文 `Dialogue:`),不用"台词："。
- 背景:SD25-01(裸标题)与 SD25-02(功能标题)两式均已验证;用户定裸标题为输出规范。Kling `【0–t｜状态】` 分段框架不受影响;已存档 fixture 保持原文不追改。
- 边界:这是输出格式规范,不构成模型效果差异的结论。
