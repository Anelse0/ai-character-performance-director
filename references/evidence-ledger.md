...
### [EXT-OBS-01] 第三方六部结构与四技巧(观察级)

- 状态：`experimental`(第三方创作者仅公开结构描述;用户观看其成片并认可效果,但无原文 prompt、无模型/条件信息、非用户生成,不满足 user_verified)
- 登记日期：2026-08-18。
- 内容:母女对峙双人戏的六部结构(references/characters/scene/dialogue/camera/performance timeline),其中四项为本 skill 尚无的技巧——(a)**人物小传块**:双人各自的性格/历史/恐惧写进 prompt 正文;(b)**走位情绪曲线**:人物间距离随张力变化(逼近/退至墙边);(c)**台词词语级触发**:情绪 beat 锚定到具体词("your father"→记忆闪回,"ENOUGH"→峰值);(d)**峰值即斩**:最高点立即切黑的收束变体。另:功能性焦段(50mm/85mm)用法与官方范本 E3(35mm/IMAX)互证。
- 允许结论:四技巧作为 experimental 手法进入技巧集,可在锚定转写中选用;在本 skill 产出中经用户实测通过后逐项升级。
- 禁止扩写:不据此宣称任何模型能力;核心原则"对白、表演、镜头是一个连续表演"与本 skill 既有立场一致,不作为新规则重复登记。
