...
# 1.8.0: 多情绪转折类型已移除，仅保留设计边界说明
grep -q '设计边界' "${lp}"
test "$(grep -c '^## 8\. 多情绪转折样例' "${lp}")" -eq 0
# 单一情绪递进原型内容不得丢失
grep -q '## 6\. 可填空模板' "${lp}"
grep -q '{{角色参考图A}}' "${lp}"
# 委托而非复制：双人形态指向已有引擎
...
grep -q '单个 Shot 内的写法' "${skill_root}/adapters/seedance-2.md"
grep -q '所有类型' "${skill_root}/adapters/seedance-2.md"
grep -q '一律用结构化分镜格式' "${skill_root}/SKILL.md"
grep -q '不限类型' "${skill_root}/SKILL.md"

# --- 1.8.1: prompt self-sufficiency + mandatory scene/visible-signal ---
grep -q '每条 Prompt 必须独立自足' "${skill_root}/SKILL.md"
grep -q '图生视频也不省略' "${skill_root}/SKILL.md"
grep -q '\[USER-PROMPT-01\]' "${skill_root}/references/evidence-ledger.md"
...
# --- 1.10.1: output label conventions (bare shot headings + 对白 label) ---
grep -q '\[USER-FORMAT-01\]' "${skill_root}/SKILL.md"
grep -q '不带说明后缀' "${skill_root}/SKILL.md"
grep -q '对白：' "${skill_root}/SKILL.md"
